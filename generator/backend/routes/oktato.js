// Auto-generated CRUD for table oktato
...
router.get('/', async (req, res) => {
  try {
    const Oktato = req.app.get('models')['oktato'];
    ...
    if (filters) {
      try {
        ...
        const conds = [];
        const attrs = Oktato.rawAttributes || {};
    ...
    const rows = await Oktato.findAll(opts);
    res.json(rows);
  } catch (e) {
    console.error(e);
...
router.get('/:id', async (req, res) => {
  try {
    const Oktato = req.app.get('models')['oktato'];
    const where = buildPkWhere(Oktato, req, res);
    if (!where) return;

    const row = await Oktato.findOne({ where });
...
router.post('/', async (req, res) => {
  try {
    const Model = req.app.get('models')['oktato'];
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const [cnt] = await Oktato.update(req.body, { where });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const cnt = await Oktato.destroy({ where });
    if (!cnt) return res.status(404).json({ error: 'NOT_FOUND' });
    res.status(204).end();
  } catch (e) {
    console.error(e);
