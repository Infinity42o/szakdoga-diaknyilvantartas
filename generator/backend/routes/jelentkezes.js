// Auto-generated CRUD for table jelentkezes
...
router.get('/', async (req, res) => {
  try {
    const Jelentkezes = req.app.get('models')['jelentkezes'];
    ...
    if (filters) {
      try {
        ...
        const conds = [];
        const attrs = Jelentkezes.rawAttributes || {};
    ...
    const rows = await Jelentkezes.findAll(opts);
    res.json(rows);
  } catch (e) {
    console.error(e);
...
router.get('/:hallgato_id/:kurzus_id', async (req, res) => {
  try {
    const Jelentkezes = req.app.get('models')['jelentkezes'];
    const where = buildPkWhere(Jelentkezes, req, res);
    if (!where) return;

    const row = await Jelentkezes.findOne({ where });
...
router.post('/', async (req, res) => {
  try {
    const Model = req.app.get('models')['jelentkezes'];
...
router.put('/:hallgato_id/:kurzus_id', async (req, res) => {
  try {
    ...
    const [cnt] = await Jelentkezes.update(req.body, { where });
...
router.delete('/:hallgato_id/:kurzus_id', async (req, res) => {
  try {
    ...
    const cnt = await Jelentkezes.destroy({ where });
    if (!cnt) return res.status(404).json({ error: 'NOT_FOUND' });
    res.status(204).end();
  } catch (e) {
    console.error(e);
